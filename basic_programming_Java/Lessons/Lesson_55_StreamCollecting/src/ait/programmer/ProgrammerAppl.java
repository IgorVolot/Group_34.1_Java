...
//            int size = programmer.getTechnologies().length; // получили ключ
//            max = Math.max(size, max);  // size > max ? size : max;
//            skilledProgrammers.putIfAbsent(size, new ArrayList<>());
////            List<Programmer> programmerList = skilledProgrammers.get(size);// вместо этого см. через строчку
////            programmerList.add(programmer);  // по ключу size добавляем как value вList нового программиста
//            skilledProgrammers.get(size).add(programmer); // по ключу size добавляем как value в List нового программиста
//        }
